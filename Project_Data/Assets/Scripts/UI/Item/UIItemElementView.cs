...
public class UIItemElementView : MonoBehaviour
{
    ...
    [SerializeField] private Text nameTextCompoent;
    [SerializeField] private Text descTextCompoent;
    ...
    {
        int spriteLength = GameManager.Instance.gameData.GetItemLength();
        int randomIndex = UnityEngine.Random.Range(0, spriteLength);

        currentItemData = GameManager.Instance.gameData.GetItemData(randomIndex);
        iconImage.sprite = currentItemData.itemIcon;
    }
}